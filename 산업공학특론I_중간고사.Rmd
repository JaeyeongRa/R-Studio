...
# 두 개의 모평균 차에 관한 검정
# 신뢰구간 상한
U_max_ap_dp = qnorm(1-(1-a_ap_dp)/2)
# 신뢰구간 하한
U_min_ap_dp = -qnorm(1-(1-a_ap_dp)/2)

# 단계4) 검정통계량 계산

dat_ap_dp = split(clth_dat_pp$actual_productivity, clth_dat_pp$department)
U_0_ap_dp = (mean(dat_ap_dp$finishing)-mean(dat_ap_dp$sweing))/(sqrt((var(dat_ap_dp$finishing)/length(dat_ap_dp$finishing))+(var(dat_ap_dp$sweing/length(dat_ap_dp$sweing)))))
