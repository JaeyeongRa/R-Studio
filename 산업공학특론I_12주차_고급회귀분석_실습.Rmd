...
dt = train(ncap_rating ~., data = trainsc, method = 'rpart', trControl = train_control, control = rpart_control, tuneGrid = dt_grid)
...
svr = train(ncap_rating ~., data = trainsc, method = 'svmRadial', trControl = train_control, tuneGrid = svm_grid)
...
mlp = train(ncap_rating ~., data = trainsc, method = 'nnet', trControl = train_control, tuneGrid = mlp_grid, linout = T, trace = F)
...
rf = train(ncap_rating ~., data = trainsc, method = 'rf', trControl = train_control)
...
xgb = train(ncap_rating ~., data = trainsc, method = 'xgbTree', trControl = train_control)
...
evaluate = function(model, test){
  ...
  pred = predict(model$finalModel, newdata = testsc)
  
  result = c(mae(pred, testsc$ncap_rating),rmse(pred, testsc$ncap_rating), mse(pred, testsc$ncap_rating))
