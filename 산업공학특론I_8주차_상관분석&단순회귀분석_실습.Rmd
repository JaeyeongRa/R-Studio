...
### 피어슨 상관계수: 선형적인 광계를 정량적으로 측정
### 켄달 상관계수: x와 Y를 순열화 하여 일치여부 판단하는 순열 척도
### 스피어만 상관계수: X와 Y를 순열화 + 비선형적인 상관관계
...
corrplot(corr_k, col = col(200), method = 'square', addCoef.col = 'black')
corrplot(corr_s, method = 'color', col = col2(200), addCoef.col = 'black', type = 'upper')
...
# 구간 내 0이 미포함되므로 귀무가설 기각, 대립가설 채택 (상관계수 0이 아님)
...
par(mfrow = c(2,2))
plot(reg) # 잔차의 정규성/등분산성 확인
{
plot(x,y, pch = 16)
abline(a = reg$coefficients[1], b = reg$coefficients[2], col = 'red') # y = a +  bx
}
