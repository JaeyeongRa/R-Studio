...
mle = function(prm){
  -sum(dexp(x, rate = prm, log = TRUE))
}


# 최적화 수행하여 모수 추정

mle_opt = optimize(mle, c(0,10)) # 지수함수라 optimize 함수 사용, parameter 2개 이상일 경우 optim 사용
...
{
hist(x, col='grey', breaks = 50, main = 'Density of Simulated Data', prob = TRUE)
curve(dexp(x, rate = mle_opt$minimum), col='red', add = TRUE)
curve(dexp(x, rate = lse_opt$minimum), col='blue', add = TRUE)
legend('topright',legend = c('MLE','LSE'), col=c('red','blue'), lty = 1, lwd = 2)
}
```
...
mle = function(prm){
  -sum(dnorm(x, mean = prm[1], sd = prm[2], log = TRUE))
}


# 최적화 수행하여 모수 추정 (2차원 이상은 optim 함수 사용)

opt = optim(c(mean(x),sd(x)), mle, hessian = TRUE) # convergence = 0 수렴이 잘 되었음
print(opt)
...
t.test(no, yes, var.equal = FALSE) # 불등분산 가정정
